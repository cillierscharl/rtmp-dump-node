...
RTMPDump.RTMPDumpCommand.prototype = {
	buildRMTPCommand : function(){
		if(!this.params.length){
			console.log('ERROR: No RTMPDump parameters specified.');
			return;
		}
		...
		for(var i = 0 ; i < this.params.length ; i++){
			if(!this.params[i].option){
				console.log('WARNING: Unmapped parameter found, your mileage may vary.');
				continue;
			}
			...
			if(this.params[i].value){
				...
				if(this.params[i].option.parameter){
					console.log('WARNING: Flag without parameter specified.');
				}
			}
		}
...
RTMPDump.outputs = {
	...
	ffmpeg : {
		command : '| {0} -i pipe:0 {1} -f flv {2}',
		defaultSettings : '-c:a copy -c:v copy',
