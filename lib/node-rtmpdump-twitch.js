...
RTMPDump.ApplicationWrappers.TwitchImplementation.prototype = {
	...
	getStreamSWFUrl : function(){
		...
		var req = http.request(RTMPDump.ApplicationWrappers.Twitch.config.twitchFlashObjectURL, function(res){
			...
			if(res.statusCode == 302){
				var swfLocation = res.headers.location.split('?')[0];
				if(!swfLocation.endsWith){
					req.end();
					RTMPDump.util.error('Unable to get SWF URL from redirect request');
				}
				RTMPDump.util.info('SWF Url Received');
				_this.swfUrl = res.headers.location.split('?')[0];
				_this.getStreamMetaData();
			}else {
				RTMPDump.util.error('Error Getting Twitch SWF.');
			}
		});

		req.on('error',function(e){
			RTMPDump.util.error('Error fetching SWF URL  : ' + e.message);
		});
		req.end();
	},
	buildRTMPDumpCommand : function(){
		...
		RTMPDump.util.successInfo('Ready! Launching Command');
